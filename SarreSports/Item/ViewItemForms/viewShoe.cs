//Project Name: SarreSports | File Name: viewShoe.cs
//Author Name: Samuel Steven David Herring
//Author Email: s.s.herring1042@canterbury.ac.uk
//Author URI: http://sherring.me
//UserID: sh1042
//Created On: 7/1/2019 | 15:44
//Last Updated On:  7/1/2019 | 15:58
using System;
...
{
    public partial class viewShoe : viewItem
    {
        private Shoe product;
        public int valueReturn { get; set; }

        public viewShoe(int productID, Branch currentBranch, viewItemState state)
        {
            InitializeComponent();

            if (!base.updateViewItemState(state) || !(currentBranch.findProduct(productID) is Shoe))
            {
            ...
            {
                product = (Shoe)currentBranch.findProduct(productID);
        ...
        private void updateSpecificAttributes(dynamic product)
        {
            if (product is Shoe)
            {
                uiShoeSizeTextBox.Text = product.Size.ToString() ?? "Not Found";
                uiShoeTypeTextBox.Text = product.ShoeType.ToString() ?? "Not Found";
            }
        }
